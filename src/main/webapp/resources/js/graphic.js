...
function drawGraphicsCarcas() {
    ...
    ctx.lineTo(MID_WIDTH_PIX + 2 * HALF_RADIUS_PIX, MID_HEIGHT_PIX - 2 * HALF_RADIUS_PIX);
    ctx.lineTo(MID_WIDTH_PIX, MID_HEIGHT_PIX - 2 * HALF_RADIUS_PIX);
    ctx.lineTo(MID_WIDTH_PIX, MID_HEIGHT_PIX);
    ctx.stroke();
    ctx.fill();

    /* 2 четверть - окружность */
    ctx.beginPath();
    ctx.moveTo(MID_WIDTH_PIX, MID_HEIGHT_PIX);
    ctx.arc(MID_WIDTH_PIX, MID_HEIGHT_PIX, HALF_RADIUS_PIX, -Math.PI, -Math.PI / 2, false);
    ctx.stroke();
    ctx.fill();
...
function drawDots(currentRadius) {
    ...
    for (let i = 0; i < resValues.length; i++) {
        ...
        let color = (resValues[i].innerHTML === "Got" ? "#66ff99" : "red");
...
function drawLastDot() {
    setTimeout(function () {
        ...
        let color = (resValues[index].innerHTML === "Got" ? "#66ff99" : "red");

        if (table_r == parseFloat(inputR)) drawDot(table_x, table_y, color);
    }, 150);}
